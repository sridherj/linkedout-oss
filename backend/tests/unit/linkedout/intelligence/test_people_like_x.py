...
@pytest.fixture()
def _mock_db():
    """Create a mock request with app.state.db_manager and embedding provider for all tests."""
...
async def test_rejects_unenriched_profiles(_mock_db):
    ...
    mock_session.execute.return_value.fetchone.return_value = ("cp_1", None, False, "John Doe")
...
async def test_rejects_enriched_but_no_embedding(_mock_db):
    ...
    mock_session.execute.return_value.fetchone.return_value = ("cp_1", None, True, "John Doe")
