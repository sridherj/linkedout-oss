...
- **CLI entry point for subprocesses**: Setup modules that invoke CLI commands use the
  installed `linkedout` entry point (`["linkedout", "embed-query", ...]`), not
  `sys.executable -m`. The entry point is installed by `pip install -e backend/` and is
  the canonical way to invoke LinkedOut commands from Python. Exception: `skill_install.py`
  uses `sys.executable` to run a Python script directly (correct). Exception:
  `logging_integration.py` reads `sys.executable` for diagnostics (correct).

- **Timestamps in raw SQL**: All raw SQL `INSERT` statements include `NOW()` for both
  `created_at` and `updated_at` columns. The ORM uses Python-side `default=` (not
  `server_default`), so raw SQL bypasses those defaults and must supply timestamps
